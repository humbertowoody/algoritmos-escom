...
{
	// double mega = 1.0e-6; // Se comenta variable por falta de uso.
...
/**
 * @brief Función para guardar el tiempo usando una estructura.
 *
 * @param tiempo* Un apuntador a la estructura tiempo donde registrar los datos.
 */
void guardar_tiempo(medicion_tiempo *tiempo)
{
  // Llamamos a la función previamente definida pero con los datos de la estructura.
  uswtime(&(tiempo->usertime), &(tiempo->systime), &(tiempo->walltime));
}

/**
 * @brief Función para calcular los resultados entre dos mediciones de tiempo.
 *
 * @param medicion_tiempo La medición de tiempo inicial.
 * @param medicion_tiempo La medicion de tiempo final.
 */
resultados_tiempo calcular_resultados(medicion_tiempo inicio, medicion_tiempo fin)
{
  // Declaración de variables.
  resultados_tiempo resultado;

  // Realizamos los cálculos.
  resultado.real = fin.walltime - inicio.walltime;
  resultado.user = fin.usertime - inicio.usertime;
  resultado.sys = fin.systime - inicio.systime;
  resultado.cpu_wall = 100.0 * (fin.usertime - inicio.usertime + fin.systime - inicio.systime) / (fin.walltime - inicio.walltime);

  // Regresamos la estructura con los datos.
  return resultado;
}
